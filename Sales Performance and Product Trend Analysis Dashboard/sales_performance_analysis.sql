...
-- I want to see which customer age range spends the most
SELECT
	CASE
...
-- I want to see which genders spend the most as well
SELECT
	gender,
...
-- How long were these customers shopping with the company? How much did they spend? What is their loyalty score?
SELECT 
	customer_id,
...
WHERE is_fraudulent = 1
GROUP BY 
	years_since_fraudulent_customers_joined, 
    customer_id, 
    avg_order_value, 
    loyalty_score
ORDER BY 
...
-- This query is to see if customers that commit fraud were close to ending their use of the company's services
SELECT
	customer_id,
    churn_risk
FROM customer_data
WHERE is_fraudulent = 1
ORDER BY churn_risk, customer_id;
...
-- Where do these customers typically reside? How much money was lost? How often did they come back?
SELECT 
	country,
    COUNT(is_fraudulent) AS num_fraudulent_customers,
    ROUND(SUM(avg_order_value), 2) AS avg_revenue_lost_by_country,
    ROUND(AVG(loyalty_score), 2) AS avg_rate_of_fraud
FROM customer_data
WHERE is_fraudulent = 1
GROUP BY country
ORDER BY 
	num_fraudulent_customers DESC, 
    avg_revenue_lost_by_country DESC;

-- I'm going to see how many fraudulent purchases were made by gender, how much was lost, and how often they came back
SELECT 
	gender, 
	SUM(total_orders) AS total_fradualent_orders_by_gender,
    ROUND(SUM(avg_order_value), 2) AS avg_revenue_lost_by_gender,
    ROUND(AVG(loyalty_score), 2) AS avg_rate_of_fraud
FROM customer_data
WHERE is_fraudulent = 1
GROUP BY gender
ORDER BY 
	total_fradualent_orders_by_gender DESC, 
    avg_revenue_lost_by_gender DESC, 
    avg_rate_of_fraud DESC;

-- How did the fraudlent orders breakdown by age? How much did they spend? How often did they come back?
SELECT 
	CASE
		WHEN age < 18 THEN 'under 18'
		WHEN age BETWEEN 18 AND 24 THEN '18-24'
    ...
    ROUND(SUM(avg_order_value), 2) AS avg_revenue_lost_by_age,
    ROUND(AVG(loyalty_score), 2) AS avg_rate_of_fraud
FROM customer_data
WHERE is_fraudulent = 1
GROUP BY 
	age_range
ORDER BY 
	total_fraudulent_orders_by_age DESC, 
	avg_revenue_lost_by_age DESC, 
    avg_rate_of_fraud DESC;
...
ORDER BY
	preferred_category, 
    total_fraudulent_orders DESC, 
    avg_revenue_lost DESC;
